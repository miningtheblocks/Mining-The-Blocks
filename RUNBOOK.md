...
| Keystore release | `mtb-release-v2.keystore` — path local NO documentado (ver env var `MTB_KEYSTORE_PATH` y Bitwarden entry "MTB keystore path"). NO en git. Alias **`mtb-release-v2`** (PKCS12, store=key password unificados por limitación del formato). **File SHA-256 post-rotation (2026-06-23, audit #9 HIGH-5)**: `bc0f6b20...62a1333d2`. Cert SHA-256 (inmutable): `BF:8F:25:AC:C3:CC:CF:9B:DA:F7:63:53:FC:E5:DE:B2:25:11:89:16:9E:1C:32:20:6E:75:52:55:4D:AB:7D:C1`. Password en Bitwarden entry **"MTB release keystore — store password v2 (rotated 2026-06-23)"** (la entry vieja "MTB release keystore — v2 (2026-06-17)" tiene el password VIEJO, marcar DEPRECATED). Reemplaza la canonical previa `@miningtheblock__miningtheblocks.jks` cuya password se perdió (2026-06-17 — ver postmortem abajo). **Backups offline (Round 2 audit #9 HIGH-6, 2026-06-23)**: (1) Hitachi `/run/media/code/datos/keystore-backup-2026-06-23/` ✓; (2) USB Kingston DT 101 G2 (extraído, guardar físicamente separado) — **PENDIENTE re-copiar post-rotation 2026-06-23**; (3) email off-site — **PENDIENTE re-enviar post-rotation**. Test recovery cada 6 meses (próximo: 2027-Q1). |
